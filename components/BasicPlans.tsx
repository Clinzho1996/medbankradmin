...
export default function BasicPlans() {
	...
	const formSubmit = async () => {
		try {
			...
			const subId = localStorage.getItem("subIdBasic");
			...
			if (data.status === "success") {
				alert("Basic plan updated successfully");
				router.push("/");
			} else {
	...
	const fetchSubscriptionPlans = async () => {
		try {
			...
			if (response.data.status === "success") {
				localStorage.setItem("subIdBasic", response.data.data[0].id);

				const basicPlan = response.data.data.find(
					(plan: SubscriptionPlan) => plan.subscription_plan_name === "Basic"
				);

				if (basicPlan) {
					const monthlyAmount = parseFloat(basicPlan.subscription_plan_amount);
					setPlans({
						monthly: {
							name: "Monthly",
							price: monthlyAmount,
				...
				<div className="bg-white rounded-lg shadow-md p-6 mb-6">
					...
					<div className="space-y-4">
						<div className="flex items-start gap-3">
							...
							<p className="text-gray-600">Connect up to 3 accounts</p>
						</div>
						<div className="flex items-start gap-3">
							<CheckCircle className="h-5 w-5 text-green-500 mt-0.5" />
							<p className="text-gray-600">Create 7 budgets per month</p>
						</div>
					</div>
				</div>
